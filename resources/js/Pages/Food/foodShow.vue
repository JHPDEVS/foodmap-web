                ...
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                    <div class="flex flex-col md:flex-row -mx-4">
                        <div class="md:flex-1 px-4">
                            ...
                            <img v-else src="/image/noimg.gif">
                        ...
                        <div class="md:flex-1 px-4">
                            <h2 v-if="title" v-html="title" class="mb-2 leading-tight tracking-tight font-bold text-gray-800 text-2xl md:text-3xl">
                               </h2>
                               <h2 v-else v-html="food.식품명" class="mb-2 leading-tight tracking-tight font-bold text-gray-800 text-2xl md:text-3xl">
                               </h2>
                            ...
                            <div class="flex items-center space-x-4 my-4">
                                <div>
                                    <div class="rounded-lg bg-gray-100 flex py-2 px-3">
                                        <span class="font-bold text-indigo-600 text-3xl">{{food.에너지_kcal_}}</span>
                                        <span class="text-indigo-400 mr-1 mt-1">칼로리</span>
                                    </div>
                                </div>
                                <div class="flex-1">
                            ...
                            <div>
                                <div v-if="items[1]" class="bg-white shadow-md rounded my-6">
                                    <table class="text-left w-full border-collapse">
                                        ...
                                        <tbody>
                                            <tr v-for="(item,i) in items" v-bind:key="i" class="hover:bg-grey-lighter">
                                                ...
                                                <td class="py-4 px-6 border-b border-grey-light "> <a :href="item.link" class="bg-green-200 text-green-600 py-1 px-3 rounded-full">사러가기</a></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </div>
...
<div class="p-1 border-4 border-black font-sans w-72">
    ...
    <div class="flex justify-between font-bold border-b-8 border-black">
        <div>1회제공량</div><div>{{food.일회제공량}}{{food.내용량_단위}}</div>
    </div>
    ...
    <div class="border-t-4 border-black text-sm pb-1">
        ...
        <hr class="border-gray-500"/>
          <div class="flex justify-between">
            <div>  <span class="font-bold">    당류</span> {{food.총당류_g_}}g</div>
        ...
        <hr class="border-gray-500"/>
          <div class="flex justify-between">
            <div>  <span class="font-bold">    트랜스지방</span> {{food.트랜스_지방산_g_}}g</div>
    ...
    <div class="border-t-8 border-black pt-1 text-sm">
        ...
        <div class="border-t-4 border-black flex leading-none text-xs pt-2 pb-1">
            <div class="pr-1">*</div>
            <div><span class="font-bold">1일 영양성분 기준치에 대한 비율(%)</span> 은 2000 kcal 기준이므로 개인의 필요 열량에 따라 다를 수 있습니다.</div>
        </div>
    </div>
</div>
                        </div>
        ...
        mounted() {
            axios.get('/food/index/' + this.food_id + '/' + this.food_name)
                .then(response => {
                    ...
                    this.dailySuger = (this.food.총당류_g_)
            ...
            getImage() {
                axios.get('https://www.ckme.live/https://openapi.naver.com/v1/search/shop.json', {
                    ...
                    .then(response => {
                        ...
                        this.maker = response.data.items[0].maker
                    })
            },
